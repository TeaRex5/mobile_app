...
// создаем температуры для выбранной комнаты
let activeRoom = 'all';
let activeTab = 'temperature';
 // делаем панель настроек комнаты
 const roomsData = {
livingroom: {
 temperature: 22,
...
// настравиваем работу выпадающего списка
dom.selectbox.querySelector('.selectbox__selected').onclick = (event) => {
    // используем toggle чтобы при нажатии меню открывалось, а другой раз закрывалась
...
// создаем код для выпадающего меню, когда нажимаем переходит на нужную вкладку
dom.selectboxList.onclick = (event) => {
    const {target} = event
    if (target.matches('.selectbox__item')) {
        // dataset в немго входит отдельный элемент data-room
        const {room} = target.dataset
        const selectedItem = dom.selectboxList.querySelector('.selected')
        // закрываем выпадающей спискок
...
// создаем функцию, которая будет выбирать нужную комнату
...
dom.rooms.querySelectorAll('.room').forEach( room => {
    // вставлеям обработчик события клика
    room.onclick = (event) => {
    ...
    function renderTemperature(temperature) {
        ...
        if (temperature >= min && temperature <= max ) {
            // меняем значени температуры
            const finishPercent = Math.round ((temperature - min) / percent);
            // меняем значение линии и круга
            ...
            // что круг двигался вместе с линией
            dom.temperatureRound.style.transform = `rotate(${roundFinishPercent}deg`;
            // добавляем чтобы менялось значение температуры
            dom.temperature.innerText = temperature;
        }
    }
...
// установка включения значения кнопки температуры
...
function changeSettingsType(type) {
    ...
    //    отслеживание изменение слайдера
...
// включение/ выключение света и влажности 
